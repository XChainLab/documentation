...
#### 一 前言
p2p模块总是在区块链的众多子模块中第一个启动，它承担将本地消息向外发送，获取外部消息的职责；获取与发送消息之前p2p模块又需要保证自身维持一个最低的链接数，维持这个最低链接数就需要获取跟多节点信息。
...
###### 3 总结
以上只是简单描述了设计几个核心模块时需要主要注意的问题以及几条建议，其他一些同样重要的问题并没有提到，比如：流量控制、节点管理、ssl transport、Conn封装、消息去重等。最后建议：p2p模块应该只是承担消息传输和节点发现的功能， 具体的业务逻辑不应该交由p2p处理，事实上主流公链也都采用如下的架构来解耦具体业务：
 + p2p负责网络读
 + conn 封装
 + 消息解析及路由
 + handler处理业务
