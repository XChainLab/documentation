...
以上只是简单描述了设计几个核心模块时需要注意的细节以及几条建议，其他一些同样重要的模块并没有提到，比如：流量控制、节点管理、ssl transport、Conn封装、消息去重等。最后建议：p2p模块应该只是承担消息传输和节点发现的功能， 具体的业务逻辑不应该交由p2p处理。主流公链几乎都采用以下的基本架构：
 + p2p负责网络读
 + conn 封装
 + 消息解析及路由
 + handler处理业务
